        ...
        __global const unsigned * restrict prefix_lengthB)
{
    ...
    for(unsigned pair=0;pair<PAIR_BLOCKS; pair+=WORKERS)
    {
        ...
        #pragma unroll 1
        for(unsigned short w=0; w<WORKERS; w+=6)
        {
        ...
        __global const unsigned * restrict prefix_lengthB)
{
    ...
    for(unsigned pair=0;pair<PAIR_BLOCKS; pair+=WORKERS)
    {
        __global char* volatile ptr_a[WORKERS];
        #pragma unroll 1
        for(unsigned short w=1; w<WORKERS; w+=6)
        {
            __global char * volatile ptr_b; //declare with volatile so that the compiler will not waste area for caching loaded data 
            unsigned seqA_idx, seqB_idx;
        ...
        __global const unsigned * restrict prefix_lengthB)
{
    ...
    for(unsigned pair=0;pair<PAIR_BLOCKS; pair+=WORKERS)
    {
        __global char* volatile ptr_a[WORKERS];
        #pragma unroll 1
        for(unsigned short w=2; w<WORKERS; w+=6)
        {
            __global char * volatile ptr_b; //declare with volatile so that the compiler will not waste area for caching loaded data 
            unsigned seqA_idx, seqB_idx;
            seqA_idx     = prefix_lengthA[pair + w - 1];
        ...
        __global const unsigned * restrict prefix_lengthB)
{
    ...
    for(unsigned pair=0;pair<PAIR_BLOCKS; pair+=WORKERS)
    {
        __global char* volatile ptr_a[WORKERS];
        #pragma unroll 1
        for(unsigned short w=3; w<WORKERS; w+=6)
        {
            __global char * volatile ptr_b; //declare with volatile so that the compiler will not waste area for caching loaded data 
            unsigned seqA_idx, seqB_idx;
            seqA_idx     = prefix_lengthA[pair + w - 1];
        ...
        __global const unsigned * restrict prefix_lengthB)
{
    ...
    for(unsigned pair=0;pair<PAIR_BLOCKS; pair+=WORKERS)
    {
        __global char* volatile ptr_a[WORKERS];
        #pragma unroll 1
        for(unsigned short w=4; w<WORKERS; w+=6)
        {
            __global char * volatile ptr_b; //declare with volatile so that the compiler will not waste area for caching loaded data 
            unsigned seqA_idx, seqB_idx;
            seqA_idx     = prefix_lengthA[pair + w - 1];
        ...
        __global const unsigned * restrict prefix_lengthB)
{
    ...
    for(unsigned pair=0;pair<PAIR_BLOCKS; pair+=WORKERS)
    {
        __global char* volatile ptr_a[WORKERS];
        #pragma unroll 1
        for(unsigned short w=5; w<WORKERS; w+=6)
        {
            __global char * volatile ptr_b; //declare with volatile so that the compiler will not waste area for caching loaded data 
            unsigned seqA_idx, seqB_idx;
            seqA_idx     = prefix_lengthA[pair + w - 1];
