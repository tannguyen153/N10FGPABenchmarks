	...
	__global short* seqB_align_begin, __global short* seqB_align_end)
{
	...
	seqA_idx         = 0;//seqA_array;
	seqB_idx         = 0;//seqB_array;
	I_i_idx          = /*I_i_array +*/ (myId * maxMatrixSize);
	I_j_idx          = /*I_j_array +*/ (myId * maxMatrixSize);
    }
    else
    {
	lengthSeqA = prefix_lengthA[myId] - prefix_lengthA[myId - 1];
	lengthSeqB = prefix_lengthB[myId] - prefix_lengthB[myId - 1];
	seqA_idx = /*seqA_array +*/ prefix_lengthA[myId - 1];
	seqB_idx = /*seqB_array +*/ prefix_lengthB[myId - 1];
	I_i_idx  = /*I_i_array +*/ (myId * maxMatrixSize);
	I_j_idx  = /*I_j_array +*/ (myId * maxMatrixSize);
    }
    ...
    __local int j_max;
    int            j            = myTId + 1;
    char           myColumnChar = seqB_array[seqB_idx+j - 1];  // read only once

    ///////////locsl dtring read in
    for(int i = myTId; i < lengthSeqA; i += 32)
    {
	myLocString[i] = seqA_array[seqA_idx+i];
    }
	...
	if(is_valid_array[myTId + is_valid_idx] && myTId < lengthSeqB)
	{
	    ...
	    //(myLocString[i-1] == myColumnChar)?MATCH:MMATCH
		...
		is_valid_array[curr_H_idx+j] =maxVal;
	    }
	    ...
	    I_i_array[I_i_idx+diagOffset[diagId] + locOffset] = i + iVal[ind];  // coalesced accesses, need to change
	    I_j_array[I_j_idx+diagOffset[diagId] + locOffset] = j + jVal[ind];
    ...
    {
	cacheVal[myTId]= thread_max;
	cacheIdx[myTId]= thread_max_i;
	cacheIdx2[myTId]= thread_max_j;
	...
	thread_max_i= cacheIdx[0];
	thread_max_j= cacheIdx2[0];
	thread_max  = cacheVal[0];
    }
	...
	//manually inline the call
	{
	    ...
	    short next_i = I_i_array[I_i_idx+diagOffset[current_diagId] + current_locOffset];
	    short next_j = I_j_array[I_j_idx+diagOffset[current_diagId] + current_locOffset];
		...
		next_i = I_i_array[I_i_idx+diagOffset[current_diagId] + current_locOffset];
		next_j = I_j_array[I_j_idx+diagOffset[current_diagId] + current_locOffset];
	    }
