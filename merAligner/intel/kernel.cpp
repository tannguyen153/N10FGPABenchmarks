                    ...
                    short* seqB_align_begin, short* seqB_align_end)
{
	...
	for(int ii=0; ii<lengthSeqB + 1; ii++)curr_H[ii]=0;
        __syncthreads();
        tmp_ptr     = prev_E;
        prev_E      = curr_E;
        curr_E      = prev_prev_E;
	...
	for(int ii=0; ii<lengthSeqB + 1; ii++)curr_E[ii]=0;
        __syncthreads();
        tmp_ptr     = prev_F;
        prev_F      = curr_F;
        curr_F      = prev_prev_F;
	...
	for(int ii=0; ii<lengthSeqB + 1; ii++)curr_F[ii]=0;

        __syncthreads();

        if(is_valid[myTId] && myTId < lengthSeqB)
        {
		int diff= is_valid-is_valid_array;
		int hdiff= curr_H-(short*)curr_H_base;
